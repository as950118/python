...
file=open("c:\\python34\\crawl\\map\\%s.txt"%search,"w",encoding='utf-8')
headers={
...
for item in items:
    file.write('-------------------------------\n')
    file.write(' 업체명 : '+ item.title.get_text(strip=1)+'\n')
    file.write(' 업체설명 : '+ item.description.get_text(strip=1)+'\n')
    file.write(' 위치 : '+ item.mapx+','+ item.mapy+'\n')
