...
X_img = tf.reshape(X, [-1, 28, 28, 1])   # img 28x28x1 (black/white), -1은 n을 의미함(상황에 따라 맞춰)
Y = tf.placeholder(tf.float32, [None, 10])

# L1 ImgIn shape=(?, 28, 28, 1)
W1 = tf.Variable(tf.random_normal([3, 3, 1, 32], stddev=0.01)) #(x, y, 색깔 수, 필터)
#    Conv     -> (?, 28, 28, 32)
#    Pool     -> (?, 14, 14, 32)
L1 = tf.nn.conv2d(X_img, W1, strides=[1, 1, 1, 1], padding='SAME')#자동으로 convolution해줌 / padding : SAME = filter 사이즈와 무관하게 convolution결과는 원래 이미지와 사이즈가 같게 해줌(원래 이미지에 패딩을 추가) ,, VALID
L1 = tf.nn.relu(L1)
...
L2 = tf.nn.max_pool(L2, ksize=[1, 2, 2, 1],
                    strides=[1, 2, 2, 1], padding='SAME')
L2_flat = tf.reshape(L2, [-1, 7 * 7 * 64])#나누어진 레이어들을 정렬하는 과정
'''
